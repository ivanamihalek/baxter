...
def run():
   """
   Seems that pubchem and PDB do not agree about the canonical smiles format - rectify that
   """
...
def test_run():
    ...
    for pubchem_id, inchi_key in inchi_keys.items():
        ...
        print(f"\t pubchem smiles {smiles[pubchem_id]}")
        print(f"\t rdkit smiles   {to_canonical(smiles[pubchem_id])} ")
